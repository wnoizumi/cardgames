...
public interface PlayingCardRepository extends JpaRepository<PlayingCard, Long> {
	...
	@Query("SELECT pc FROM PlayingCard pc LEFT JOIN FETCH pc.game g LEFT JOIN FETCH pc.player p WHERE g.uuid = :gameuuid AND p IS NULL ORDER BY pc.shufflePosition DESC")
	...
	@Query("SELECT pc FROM PlayingCard pc LEFT JOIN FETCH pc.player p JOIN FETCH pc.game g "
			+ "WHERE pc.uuid = :uuid")
	Optional<PlayingCard> findOneWithRelationsByUuid(@Param("uuid") UUID uuid);
}
